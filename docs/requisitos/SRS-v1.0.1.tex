% =============================================================================
% SRS v1.0.1 -- Sistema de Gestion de Pre-Sustentaciones UTEQ
% ISO/IEC/IEEE 29148:2018
% Compilar con: latexmk -pdf -interaction=nonstopmode -halt-on-error SRS-v1.0.1.tex
% =============================================================================
...
\hypersetup{colorlinks=true, linkcolor=blue, urlcolor=blue, citecolor=blue,
            pdftitle={SRS v1.0.1 - Sistema de Gestion de Pre-Sustentaciones UTEQ}}

\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small SRS v1.0.1 --- Sistema de Gestión de Pre-Sustentaciones UTEQ}
...
{\Large \textbf{Versión 1.0.1}}\\[0.9cm]
...
\textbf{Rama principal al cierre:} & \emph{commit} \texttt{3e7069c} (2026-09-09) \\
\textbf{DOI del software:} & \href{https://doi.org/10.5281/zenodo.22445216}{10.5281/zenodo.22445216} \\
\end{tabular}\normalsize\\[1.0cm]

{\large \textbf{AUTOR:}}\\[0.3cm]
{\large Álava Alvarado, Jean Pierre}\\[0.1cm]
{\large \href{https://orcid.org/0009-0001-2878-2919}{ORCID: 0009-0001-2878-2919}}\\[0.8cm]

{\large \textbf{DOCENTE-DIRECTOR:}}\\[0.3cm]
{\large Ing. Guerrero Ulloa Gleiston Cíceron, Mg.}\\[0.8cm]

{\large \textbf{FECHA DE EMISIÓN:} 11 de septiembre de 2026}\\[0.3cm]
\end{titlepage}
...
\midrule
\endhead
1.0.0 & 2026-09-08 & J. P. Álava Alvarado &
\textbf{Primera versión de esta especificación.} Corpus de 64 requisitos funcionales y 26 no
funcionales, redactados y verificados contra el código real de la etiqueta \texttt{v1.0.1}.
Incorpora la plantilla completa por requisito y las cinco secciones estructurales que la norma
exige. Entregada a revisión del docente-director. \\
\addlinespace
1.0.1 & 2026-09-11 & J. P. Álava Alvarado &
\textbf{Incorpora la revisión del docente-director sobre la v1.0.0} (8 observaciones de
modificación y 4 de adición). El corpus no cambia: cambian las fuentes declaradas, la redacción
de la cláusula de verificación, la portada y la trazabilidad de las cifras. La respuesta punto por
punto está en la sección~\ref{sec:respuesta}. \\
\bottomrule
...
\noindent\textbf{Nota de alcance sobre la autoría.} Esta especificación fue elaborada de forma
individual por Jean Pierre Álava Alvarado. El sistema que describe se construyó en el marco de un
trabajo de equipo y el repositorio mantiene el historial completo de todas las personas que
contribuyeron código; lo que esta versión declara es la autoría del \emph{documento de
requisitos}, no la del software. Se hace constar aquí, y no en la portada, porque la portada
identifica a quien responde por el documento y nada más.
...
\texttt{3e7069c} (2026-09-09), que es el punto sobre el que se emitió la revisión del
docente-director. Las cifras que dependen de una medición traen la fecha de esa
...
técnico entre la coordinación académica, los docentes, los estudiantes y quien desarrolla el
sistema, y como base de verificación: cada requisito declara cómo se comprueba y en qué
estado está.
...
\textbf{Tamaño real del sistema en el punto de corte:} 31 controladores REST \emph{de
producción} ---49 si se cuentan también las 18 clases de prueba de controlador, que no exponen
ninguna ruta--- con 207 combinaciones de método y ruta, 10 rutinas SQL (8 procedimientos y 2 funciones de auditoría), 30 migraciones
...
\campo{Prioridad}{Must}
\campo{Fuente}{OWASP A07 (recuperación de credenciales). Carencia detectada al contrastar el asistente virtual (RF-44), que ya ofrece esta capacidad al usuario, con los endpoints realmente expuestos por el módulo de autenticación.}
\campo{Endpoints}{\cd{POST /api/v1/auth/recuperar}, \cd{POST /api/v1/auth/restablecer}}
...
\campo{Nota}{Construido en la fase 6 (2026-09-11), con una decisión honesta y declarada sobre el
obstáculo real: \texttt{EmailService} existe pero \texttt{app.mail.enabled=false} por omisión, así
que no había forma de entregar el elemento de un solo uso sin activar SMTP de verdad. Se optó
deliberadamente por \textbf{no} declarar el requisito \estadoV{} sin haber comprobado nunca que el
correo llega -- la salida deshonesta que el propio encargo de esta fase prohíbe. En su lugar: el
flujo completo se construyó y se probó de punta a punta con el envío apagado (\texttt{PasswordRecoveryServiceTest},
6 casos: token con TTL de 30 min, un solo uso, aplica RNF-06, revoca \emph{todas} las sesiones sin
excepción a diferencia de RF-06; más los casos de \cd{AuthControllerIntegrationTest} sobre el
mismo cuerpo/código de respuesta exista o no la cuenta, y el límite de tasa con su fail-closed de
RNF-04), y se declara \estadoI{}, no \estadoV{}: la entrega real del correo por SMTP nunca se
verificó contra un servidor real. Activarla y comprobarla es lo que falta para el cierre completo.}
...
\campo{Prioridad}{Must}
\campo{Fuente}{OWASP A07 y NIST SP 800-63B: el titular debe poder cambiar su propia credencial. Carencia detectada al contrastar RF-04 y RF-08 con los endpoints reales.}
\campo{Endpoint}{\cd{PATCH /api/v1/auth/usuarios/\{id\}/password}}
...
\begin{itemize}
    ...
    \item El cambio revoca todas las sesiones activas del titular salvo aquella desde la que se
          realiza.
    \item Solo el titular cambia su propia contraseña: sobre otro usuario responde \texttt{403},
          incluso si quien lo intenta es Administrador (mismo patrón que RF-10 con el perfil).
    \item Se almacena conforme a RNF-05 (BCrypt, factor de coste $\geq 10$) y no se devuelve
          nunca, ni la vigente ni la nueva, en ninguna respuesta.
\end{itemize}
\campo{Método de verificación}{Prueba}
\campo{Estado}{\estadoV}
...
\campo{Prioridad}{Should}
\campo{Fuente}{OWASP A01; hallazgo de control de acceso de la auditoría del 2026-08-11/12, que separó el correo institucional (identificador de cuenta) de los datos de contacto del titular.}
...
\campo{Prioridad}{Should}
\campo{Fuente}{Reglamento de titulación: detención del proceso por decisión académica. Modelado en la migración \cd{V1\_\_schema\_inicial.sql} (campos \cd{motivo\_suspension} y \cd{suspendido\_en} de la solicitud) y permiso \cd{SOLICITUDES\_SUSPENDER} de \cd{V13\_\_permisos\_roles\_dinamicos.sql}.}
...
\campo{Prioridad}{Must}
\campo{Fuente}{Reglamento de titulación: tres revisiones obligatorias antes de conformar el tribunal. Modelado en \cd{V1\_\_schema\_inicial.sql} (tabla \cd{tutoria\_fases}).}
...
\campo{Prioridad}{Must}
\campo{Fuente}{Reglamento de titulación: entrega de correcciones por fase. Modelado en \cd{V1\_\_schema\_inicial.sql} (campos \cd{archivo\_pdf\_estudiante}, \cd{sha256\_pdf} y \cd{tamano\_pdf\_bytes} de la fase).}
...
leídos.}
\campo{Prioridad}{Should}
\campo{Fuente}{Migración \cd{V1\_\_schema\_inicial.sql}: tabla \cd{tutoria\_mensajes} y catálogo \cd{tipos\_mensaje}, que fijan el conjunto cerrado de tipos de mensaje.}
...
\campo{Prioridad}{Must}
\campo{Fuente}{Reglamento de titulación: el cierre de la tutoría es precondición de la conformación del tribunal (RF-27).}
...
\campo{Prioridad}{Should}
\campo{Fuente}{Migración \cd{V19\_\_historial\_actas\_y\_reportes.sql}, que introduce los permisos \cd{ACTAS\_VER} y \cd{ACTAS\_VER\_PROPIAS} precisamente para separar el listado completo del listado propio; OWASP A01.}
...
\campo{Prioridad}{Should}
\campo{Fuente}{Migración \cd{V19\_\_historial\_actas\_y\_reportes.sql}: catálogo \cd{estados\_acta} con su campo \cd{orden}, que es lo que define la máquina de estados de la sección~\ref{sec:estados}.}
...
\campo{Prioridad}{Should}
\campo{Fuente}{Migración \cd{V19\_\_historial\_actas\_y\_reportes.sql}: tabla \cd{historial\_estados\_acta} y permiso \cd{ACTA\_HISTORIAL\_VER}.}
...
\campo{Prioridad}{Should}
\campo{Fuente}{ADR-001: la actualización de estado se resuelve por sondeo periódico y no por canal persistente, decisión que además determina la directiva \cd{connect-src} de RNF-10.}
...
\campo{Prioridad}{Could}
\campo{Fuente}{Observación docente OBS-17 (\cd{docs/observaciones/OBSERVACIONES.md}), que exigió someter el asistente al control de acceso del resto de la API.}
...
\campo{Prioridad}{Should}
\campo{Fuente}{\cd{docs/funcionalidades/CENTRO-ORIENTACION.md} y migraciones \cd{V20} a \cd{V23}, que crean el módulo y siembran su catálogo.}
...
\campo{Prioridad}{Could}
\campo{Fuente}{\cd{docs/funcionalidades/CENTRO-ORIENTACION.md}: el objetivo declarado del módulo es que el estudiante explore ideas \emph{antes} de registrar su solicitud.}
...
\campo{Prioridad}{Could}
\campo{Fuente}{\cd{docs/funcionalidades/CENTRO-ORIENTACION.md} y migración \cd{V21}, que siembra el catálogo de temas sobre el que se guarda.}
...
de titulación.}
\campo{Prioridad}{Should}
\campo{Fuente}{Migración \cd{V21\_\_orientacion\_permisos\_y\_semilla\_temas.sql}, que crea el permiso \cd{ORIENTACION\_CATALOGO\_GESTIONAR} para curar el catálogo que RF-46 da por hecho.}
...
\campo{Prioridad}{Could}
\campo{Fuente}{Migración \cd{V23\_\_orientacion\_semilla\_recursos\_titulacion.sql}.}
...
\campo{Prioridad}{Should}
\campo{Fuente}{\cd{docs/funcionalidades/CENTRO-ORIENTACION.md} y migración \cd{V20}, que define el catálogo de pasos de la ruta de titulación.}
...
\campo{Prioridad}{Must}
\campo{Fuente}{Migración \cd{V17\_\_gestion\_carreras.sql}, que crea el permiso \cd{CARRERAS\_GESTIONAR} y lo reserva al Administrador.}
...
\campo{Prioridad}{Must}
\campo{Fuente}{Migración \cd{V16\_\_gestion\_estudiantes.sql}, que crea el permiso \cd{ESTUDIANTES\_GESTIONAR} y el catálogo de estados académicos.}
...
\campo{Prioridad}{Could}
\campo{Fuente}{ADR-006 y \cd{docs/basedatos/CATALOGO-SP.md}: el procedimiento almacenado \cd{sp\_obtener\_estadisticas\_tutores} existe para resolver esta consulta en la base de datos.}
...
\campo{Prioridad}{Could}
\campo{Fuente}{HU-02 y migración \cd{V1\_\_schema\_inicial.sql} (campo \cd{observaciones} de la solicitud), que es donde se acumulan las observaciones que RF-15, RF-21 y RF-25 producen en momentos distintos.}
...
descargarlos.}
\campo{Prioridad}{Must}
\campo{Fuente}{\cd{docs/despliegue/BACKUP.md}, criterio de continuidad de la asignatura, y migración \cd{V27\_\_permiso\_gestion\_respaldos.sql}.}
...
\campo{Prioridad}{Should}
\campo{Fuente}{\cd{docs/despliegue/BACKUP.md} y migración \cd{V28\_\_gestion\_respaldos\_cronograma.sql}, que persiste el cronograma y la política de retención como configuración.}
...
\campo{Prioridad}{Must}
\campo{Fuente}{\cd{docs/despliegue/BACKUP.md} y migración \cd{V29\_\_respaldos\_fase2\_wal\_diferencial.sql}, que registra el resultado de cada prueba de restauración.}
...
\campo{Prioridad}{Should}
\campo{Fuente}{\cd{docs/despliegue/BACKUP.md} y migración \cd{V29\_\_respaldos\_fase2\_wal\_diferencial.sql}.}
...
\campo{Método de verificación}{Prueba}
\campo{Estado}{\estadoV}
\campo{Nota}{Cerrado en la fase 4 (2026-09-11): \texttt{JwtTokenProvider.isTokenBlacklisted()} y
\texttt{RateLimiterService.isAllowed()} distinguen ahora una excepción de acceso al almacén
(\texttt{DataAccessException} y afines) de un problema del propio token o de una entrada
normal, y fallan \textbf{cerrado} (401 / 503) solo ante la primera. La caché de lectura usa un
\texttt{CacheErrorHandler} dedicado que absorbe el error y dejar seguir el método hacia el
origen -- la única de las tres que falla \textbf{abierto}, a propósito. Las tres rutas registran
una entrada identificable (\texttt{DEGRADACION (RNF-04): ...}). Verificado con
\texttt{RedisDegradacionTest.java}, doble de \texttt{StringRedisTemplate} que lanza
\texttt{RedisConnectionFailureException}.}
...
\campo{Método de verificación}{Prueba}
\campo{Estado}{\estadoV}
\campo{Nota}{Cerrado en los tres puntos de aplicación que el criterio exige, con el mismo
\texttt{PasswordPolicyValidator} reutilizable (respaldado por
\texttt{backend/src/main/resources/security/common-passwords.txt}) en los tres: el alta (RF-04,
fase 3), el cambio propio (RF-06, fase 5) y el restablecimiento (RF-05, fase 6). Nota aparte
sobre RF-05: se declara \estadoI{} y no \estadoV{} por un motivo distinto a este requisito -- la
entrega real del correo por SMTP nunca se verificó -- pero la aplicación de esta política dentro
de ese flujo sí es real y está probada (\cd{PasswordRecoveryServiceTest}), independientemente de
si el correo llega.}
...
\campo{Prioridad}{Must} \quad \campo{Fuente}{OWASP A05 y A07; \textbf{observación docente
OBS-11} (\cd{docs/observaciones/OBSERVACIONES.md}), registrada como \emph{resuelta} desde la
fase 4 (2026-09-11): los seis artefactos que repetían la credencial, incluido el origen en
código, quedan corregidos.}
\campo{Rationale}{Una cuenta de administración con contraseña conocida es equivalente a no tener
autenticación. El repositorio es público, de modo que cualquier contraseña escrita en cualquier
archivo versionado está publicada, y el historial de Git conserva lo que se retire hoy. OBS-11 ya
identificó el problema; convertirlo en requisito con criterio verificable es lo que le puso
condición de cierre a una observación que llevó meses en estado parcial.}
\campo{Criterio de aceptación}{}
\begin{itemize}
    ...
    \item \textbf{Ninguna credencial de ejemplo aparece en ningún artefacto versionado.} Una
          búsqueda de la contraseña sembrada sobre el árbol completo del repositorio ---código,
          documentación, historias de usuario, colección de peticiones y ficheros de
          configuración--- devuelve cero coincidencias.
    \item Los artefactos que necesitan una credencial para ser ejecutables (la colección de
          peticiones de la API, los ejemplos de las historias de usuario) la toman de una
          variable de entorno o de un marcador de posición, nunca de un valor literal.
\end{itemize}
\campo{Método de verificación}{Inspección + Demostración}
\campo{Estado}{\estadoV}
\campo{Nota}{\textbf{Inventario completo (verificado el 2026-09-11).} La credencial sembrada
aparecía en seis artefactos versionados del repositorio público: el código que siembra los datos
de demostración, la colección de peticiones de \cd{docs/postman/}, el escenario de ejemplo de
\cd{docs/requisitos/historias/HU-01.md}, el procedimiento de medición de \cd{k6/README.md}, la
evidencia reproducible de \cd{docs/mediciones/sec/owasp/OWASP-AUDIT.md} y el propio registro de
OBS-11. Los cinco artefactos de documentación se corrigieron primero (toman la credencial de una
variable de entorno o de una variable de colección); el origen --el código-- se cierra en esta
fase: la siembra se movió a \texttt{DemoDataSeeder}, activo \textbf{solo} bajo el perfil
\texttt{dev} (nunca en el arranque por omisión); sus contraseñas se rotaron y ya cumplen RNF-06;
y un despliegue fuera de ese perfil sin \texttt{ADMIN\_BOOTSTRAP\_EMAIL}/
\texttt{ADMIN\_BOOTSTRAP\_PASSWORD} en el entorno falla al arrancar en \texttt{AdminBootstrap}
en vez de crear una cuenta con contraseña conocida --mismo patrón que \texttt{JWT\_SECRET}
(RNF-14)--, y la contraseña que sí se provee se valida contra \texttt{PasswordPolicyValidator}
(RNF-06) antes de usarse. Verificado a mano arrancando sin perfil \texttt{dev} y sin las
variables (falla explícitamente), con una contraseña de la lista de comunes (también falla) y
con una contraseña válida (arranca y crea la cuenta). \texttt{grep -rn
"admin123\textbackslash|docente123\textbackslash|estudiante123" --include=*.java --include=*.md
--include=*.json --include=*.yml .} (excluyendo \cd{docs/requisitos/historico/}) da cero
resultados.

\textbf{Lo que sigue abierto, y por qué (sin cambios respecto a la versión anterior de esta
nota).} La migración \cd{V11\_\_roles\_y\_privilegios.sql} crea dos roles de base de datos con
contraseña literal; una migración ya aplicada no se puede editar sin romper la suma de
verificación de Flyway, así que cerrarla exige una migración nueva que las rote, no una
corrección en su sitio -- deliberadamente sin tocar en esta fase. Y esto no lo arregla ningún
cambio de código: \textbf{el historial de Git conserva las contraseñas retiradas hoy} (incluidas
las que \texttt{DemoDataSeeder} sembraba antes de esta fase), de modo que deben considerarse
comprometidas y rotarse en cualquier despliegue real; no se reescribe el historial para
ocultarlas.}
...
\campo{Endpoints}{\cd{POST /api/usuarios/\{id\}/solicitar-supresion},
\cd{GET /api/usuarios/solicitudes-supresion}, \cd{POST /api/usuarios/solicitudes-supresion/\{id\}/resolver}
-- la depuración automática de la bitácora no tiene endpoint, por diseño (ver Nota).}
...
\campo{Método de verificación}{Inspección + Demostración}
\campo{Estado}{\estadoI}
\campo{Nota}{Construido en la fase 7 (2026-09-11), 3 de 4 criterios -- se declara \estadoI{} y no
\estadoV{} porque el criterio exige los cuatro. (1) Tabla de retención publicada en
\cd{docs/etica/RETENCION-DATOS.md}, enlazada desde \cd{ETHICS.md}, con período declarado por
categoría y una distinción explícita de qué NO es suprimible (expediente académico, calificaciones,
actas) por obligación legal de conservación. (2) Depuración automática de la bitácora
(\cd{DepuracionBitacoraScheduler}, \cd{@Scheduled} mensual, retención de 2 años) sin ningún
endpoint -- resuelve la tensión con RNF-18 (ninguna operación de la API puede borrar la bitácora)
a nivel de tarea programada, no de código de aplicación alcanzable por una petición. Deja traza
verificable en \cd{presus.depuracion\_bitacora\_log} (cuántas entradas, hasta qué fecha).
(3) Procedimiento de supresión (\cd{SupresionDatosService}) que \textbf{seudonimiza}, no borra: el
expediente sigue enlazado al mismo id. El registro de la solicitud
(\cd{presus.solicitud\_supresion}) no guarda el dato suprimido, solo el id del usuario --
resolviendo la paradoja de que el registro de una supresión no puede contener lo suprimido. (4)
Sin construir de nuevo: los consentimientos firmados conservados $\geq 2$ años ya estaban
declarados como política en \cd{ETHICS.md} \S3 antes de esta fase; no se fabrica ningún
formulario firmado que no exista para completar el criterio -- sigue siendo, honestamente, una
política sin datos reales todavía que aplicarle.}
...
anterior del proyecto publicó una media de 91,25 con diez participantes que \textbf{nunca
...
Ninguna versión de este documento se publica si el validador de trazabilidad
(\cd{scripts/validate-traceability.sh}) no termina con código de salida 0. El validador es la
garantía automática de que este SRS, la matriz y el árbol de archivos describen el mismo sistema;
la revisión manual no puede sostener esa coherencia sobre 90 requisitos y 207 rutas.

\textbf{Qué garantiza hoy, y desde cuándo.} Las ocho comprobaciones de la tabla siguiente están
\textbf{implementadas y activas desde el 2026-09-11}, y son las que el script ejecuta. Conviene
ser explícito sobre el punto de partida: en la versión 1.0.0 de este documento la tabla enunciaba
las ocho comprobaciones como especificación de lo que el validador \emph{debía} hacer, mientras
que el script publicado en el repositorio solo comprobaba que los archivos citados por cada fila
existieran, y nunca abría el SRS. La revisión del docente-director señaló con razón esa distancia
entre lo enunciado y lo construido (M4). Esta versión la cierra: el validador lee el SRS, lo
contrasta con la matriz en ambos sentidos y falla si discrepan.

\textbf{Lo que el validador no garantiza.} Que el enunciado de un requisito sea correcto, que su
criterio de aceptación sea el adecuado o que la evidencia citada demuestre lo que dice demostrar.
Eso es revisión humana y ninguna comprobación automática lo sustituye. El validador garantiza
coherencia entre artefactos, no verdad de su contenido.

\begin{table}[H]\centering\small
\begin{tabular}{p{0.8cm} p{13.9cm}}
\toprule
\textbf{\#} & \textbf{Comprobación que el validador realiza} \\
\midrule
...
\bottomrule
\end{tabular}
\caption{Las ocho comprobaciones, implementadas y activas desde el 2026-09-11. V1 a V6 son las
que habrían detectado, antes de la entrega, los defectos que la sección~\ref{sec:cambios}
documenta, y V3 es la que convierte una matriz incompleta en un error de construcción en vez de
en un hallazgo de auditoría.}
\end{table}

\subsection{Salida del validador sobre este árbol}
\label{sec:salida-validador}

Reproducible con \cd{./scripts/validate-traceability.sh} desde la raíz del repositorio:

\begin{quote}\ttfamily\small\noindent
Requisitos en el SRS \dotfill{} 90\\
Filas en la matriz \dotfill{} 90\\
Endpoints reales del backend \dotfill{} 213\\
Clases de prueba en disco \dotfill{} 53\\
Estados: Implementado 14, No cumplido 3, Planificado 2, Verificado 71\\
Must verificados \dotfill{} 47/50 (94\,\%)\\
=== V1--V8: sin fallos. Trazabilidad consistente. ===
\end{quote}

\noindent Todas las cifras de la sección~\ref{sec:cumplimiento} son \textbf{esta} salida, no un
conteo manual sobre el documento. Si el documento y la matriz dejan de coincidir, el validador
falla y la cifra no llega a publicarse.
...
\textbf{Total} \\
\midrule
Requisitos funcionales     & 51 & 13 & 0 & 0 & \textbf{64} \\
Requisitos no funcionales  & 20 &  1 & 2 & 3 & \textbf{26} \\
\midrule
\textbf{Total}             & \textbf{71} & \textbf{14} & \textbf{2} & \textbf{3} &
\textbf{90} \\
\bottomrule
\end{tabular}
\caption{Distribución por estado. Cifras producidas por el validador sobre la matriz completa
(sección~\ref{sec:salida-validador}), no contadas a mano sobre el documento.}
\end{table}
...
De los \textbf{50} requisitos de prioridad \emph{Must}, \textbf{47 están verificados
(94,0\,\%)}. Los \textbf{3} restantes se enumeran aquí de forma completa, porque un porcentaje
...
\midrule
\endhead
RF-05 & Recuperación de contraseña & \estadoI & Activar SMTP real y verificar que el correo de
recuperación llega de verdad (hoy \texttt{app.mail.enabled=false}). \\
RNF-19 & Retención y supresión de datos & \estadoI & Conservar formularios de consentimiento
firmados reales cuando existan (política ya declarada; hoy no hay ninguno que conservar). \\
RNF-11 & Transporte cifrado & \estadoP & Depende del despliegue público, aún no realizado. \\
\bottomrule
\caption{Los 3 requisitos \emph{Must} no verificados, con lo que falta en cada uno.}
\end{longtable}

\noindent Ninguno de los tres exige ya cambiar código: los tres son de evidencia, configuración o
despliegue -- RF-05 (activar SMTP y verificar la entrega), RNF-11 (depende del despliegue
público, aún no realizado) y RNF-19 (conservar formularios de consentimiento reales cuando
existan; la política ya está declarada). Los tres, junto con el resto de la deuda declarada,
siguen el orden de la sección~\ref{sec:pendientes}.
...
Esta sección documenta qué se corrigió y por qué, para que la bitácora de cambios no sea la única
constancia. Cubre dos ciclos distintos: la respuesta a la revisión del docente-director sobre la
versión 1.0.0 (\S\ref{sec:respuesta}) y las correcciones que la propia 1.0.0 hizo sobre los
borradores que la precedieron (\S12.2 en adelante). El detalle por requisito está en
\cd{docs/requisitos/CHANGELOG-REQ.md}.

\subsection{Respuesta a la revisión del docente-director sobre la versión 1.0.0}
\label{sec:respuesta}

La versión 1.0.0 fue revisada por el docente-director, que emitió ocho observaciones de
modificación y cuatro de adición. Esta subsección responde a cada una: qué señalaba, qué se hizo y
dónde queda la constancia. Cuando la observación era inexacta se dice también eso, con el dato
que la corrige, porque aceptar en silencio una observación que no corresponde deja el documento
peor de lo que estaba.

\textbf{Causa raíz de cinco de las ocho observaciones.} M1 a M5 señalaban que la matriz de
trazabilidad cubría 15 de los 90 requisitos, con nueve columnas en vez de once y dos valores de
estado en vez de cuatro, y que el validador no realizaba ninguna de las comprobaciones V1 a V8.
Las cinco eran correctas, y las cinco tenían el mismo origen: \textbf{la matriz y el validador que
esta especificación describe nunca llegaron a la rama principal del repositorio}. La revisión se
hizo, con toda razón, sobre lo que había publicado ---los artefactos del borrador del
2026-08-17---, mientras el documento revisado era el nuevo. No era una discrepancia entre lo
escrito y lo construido: era una entrega incompleta. Los artefactos se publican con esta versión,
y la comprobación V3 convierte esa clase de desfase en un error de construcción.

\begin{longtable}{p{1.1cm} p{5.3cm} p{8.1cm}}
\toprule
\textbf{Obs.} & \textbf{Qué señalaba} & \textbf{Qué se hizo} \\
\midrule
\endfirsthead
\toprule
\textbf{Obs.} & \textbf{Qué señalaba} & \textbf{Qué se hizo} \\
\midrule
\endhead
M1 / A1 & La matriz cubre 15 de 90 requisitos. & \textbf{Aceptada.} Se publica la matriz completa:
90 filas, una por requisito, con método de verificación y evidencia. \\
M2 & La matriz descrita (11 columnas) no es la que existe (9 columnas con otros nombres). &
\textbf{Aceptada.} El archivo publicado tiene exactamente las once columnas que describe
\S\ref{sec:trazabilidad}, incluida \texttt{Observaciones}. \\
M3 & El vocabulario cerrado de cuatro estados no existe en la matriz, que solo usa dos. &
\textbf{Aceptada.} La columna \texttt{Estado} usa los cuatro valores de
\S\ref{sec:estados-req}, y la comprobación V2 rechaza cualquier otro. \\
M4 / A2 & El validador no realiza ninguna de las comprobaciones V1 a V8; nunca abre el SRS. &
\textbf{Aceptada.} Las ocho están implementadas y activas. La cláusula de
\S\ref{sec:trazabilidad} se reescribió para declarar qué garantiza el validador, desde cuándo y
qué no garantiza. \\
M5 & La distribución de estados no es reproducible desde el repositorio. &
\textbf{Aceptada.} Las cifras son ahora la salida literal del validador
(\S\ref{sec:salida-validador}), con el comando que las reproduce. \\
\addlinespace
M6 & Se citan HU-13, HU-14 y HU-15, que no existen. &
\textbf{Aceptada en parte.} Era cierto en la matriz publicada, ya sustituida. \textbf{No} lo era
en el documento: la única mención a esas tres historias estaba en \S12.2, describiendo un defecto
ya corregido. Esa frase se reformuló para que no pueda leerse como cita viva. \\
M7 / A3 & Con 64 requisitos y 12 historias, la mayoría del corpus no tiene origen declarado. &
\textbf{Aceptada en el fondo, matizada en la forma.} Ningún requisito tenía el campo Fuente
vacío, pero 27 declaraban «historia de usuario pendiente», que es una promesa y no una fuente.
Los 27 declaran ahora su origen real y verificable: la migración que creó su tabla o su permiso,
la decisión de arquitectura, el control OWASP, el reglamento de titulación o la observación
docente que lo motivó. Ver \S\ref{sec:pendientes}. \\
\addlinespace
M8 & La autoría del documento no está reconciliada con el historial del repositorio, y la frase de
la portada no debería estar ahí. & \textbf{Aceptada.} La portada identifica al autor y su ORCID,
sin ninguna otra declaración. La situación del equipo se explica como nota de alcance en
\S1.3, y las menciones a «el equipo» que quedaban en el cuerpo se reconciliaron. \\
\addlinespace
A4 & Falta un requisito que fije qué ocurre con las credenciales de ejemplo. &
\textbf{Aceptada como refuerzo.} El requisito ya existía ---RNF-15, en estado \estadoN{}--- pero
solo cubría el código fuente. Se amplió con dos criterios que alcanzan todo artefacto versionado,
y se le declaró como fuente la observación docente OBS-11, que sigue abierta en parte desde la
Entrega 3. \\
\bottomrule
\caption{Respuesta a las doce observaciones de la revisión de la versión 1.0.0.}
\end{longtable}

\noindent\textbf{Una precisión sobre una cifra de la revisión.} La revisión da por buena la cifra
de «207 rutas en 49 controladores». Las 207 rutas son correctas; los 49 controladores salen de
contar los 31 de \cd{src/main/java} más las 18 clases de prueba de controlador de
\cd{src/test}, que no exponen ninguna ruta. El par que este documento sostiene es \textbf{31
controladores de producción y 207 rutas}, y así queda aclarado en \S3.1 para que la diferencia no
vuelva a aparecer.

\subsection{Defectos de trazabilidad corregidos en la versión 1.0.0}
...
\begin{enumerate}
    ...
    \item \textbf{Tres historias de usuario citadas no existían como archivo.} La matriz de la
          especificación anterior referenciaba HU-13, HU-14 y HU-15, y el directorio contiene
          únicamente HU-01 a HU-12. \textbf{Ya corregido: ni este documento ni la matriz que lo
          acompaña citan hoy ninguna de las tres}, y la mención que se hace de ellas en este
          párrafo describe un defecto cerrado, no una referencia viva. La comprobación V6 del
          validador lo impide en adelante.
...
Salvo donde se marca explícitamente cerrado, nada de lo que sigue está resuelto. Se enumera de
forma completa porque un hallazgo sin registro no tiene fecha de cierre, y porque declararlo es
preferible a que aparezca en una revisión externa. La numeración se conserva estable a través de
las fases de cierre (no se renumeran los puntos ya cerrados) para que las referencias a "el punto
N" en el historial de cambios sigan siendo válidas.

\subsection{Orden de trabajo propuesto}

\begin{enumerate}
    \item \textbf{Seguridad inmediata --- cerrado en la fase 4 (2026-09-11).} RNF-15
          (contraseñas literales en el código de un repositorio público y siembra de cuentas en
          todo arranque) y RNF-04 (la revocación de tokens era \emph{fail-open}). Eran los dos
          únicos hallazgos que degradaban la seguridad del sistema desplegado; ambos se cerraron
          con un cambio acotado (\texttt{DemoDataSeeder}/\texttt{AdminBootstrap} para RNF-15,
          fail-closed explícito en revocación y límite de tasa para RNF-04). Se conserva este
          punto, marcado como resuelto, para no renumerar los siguientes.
    \item \textbf{Credenciales del usuario --- cerrado en la fase 6 (2026-09-11).} RF-06
          (cambio propio, fase 5) y RF-05 (recuperación sin sesión, fase 6) ya existen como
          funcionalidad real y probada, y RNF-06 (política de contraseñas) se aplica en los tres
          puntos que su criterio exige: alta, cambio y restablecimiento. Único cabo suelto,
          declarado y no oculto: RF-05 queda \estadoI{}, no \estadoV{}, porque la entrega real
          del correo de recuperación nunca se verificó contra un servidor SMTP (ver su nota) --
          activarlo y comprobarlo es un pendiente aparte, de infraestructura, no de
          construcción. Se conserva este punto, marcado como resuelto, para no renumerar los
          siguientes.
    \item \textbf{Protección de datos personales --- 3 de 4 criterios cerrados en la fase 7
          (2026-09-11).} RNF-19: tabla de retención publicada
          (\cd{docs/etica/RETENCION-DATOS.md}), depuración automática de la bitácora sin
          endpoint (resuelve la tensión con RNF-18) y procedimiento de supresión por
          seudonimización, los tres construidos y probados. Sigue abierto, y es genuinamente de
          infraestructura/proceso, no de código: no existe todavía ningún formulario de
          consentimiento firmado real que conservar (la política de conservación de 2 años ya
          está declarada en \cd{ETHICS.md} \S3 desde antes de esta fase). Por eso RNF-19 queda
          \estadoI{}, no \estadoV{}. Se conserva este punto para no renumerar los siguientes.
...
\subsection{Origen de los requisitos sin historia de usuario}

Treinta y uno de los 64 requisitos funcionales tienen una historia de usuario formal (HU-01 a
HU-12); los otros 33, más los 26 no funcionales, no la tienen. Eso \textbf{no} significa que
carezcan de origen: en la versión 1.0.0 declaraban «auditoría de cobertura, historia de usuario
pendiente», que es una promesa y no una fuente, y esta versión sustituye esa fórmula por el
\textbf{origen real y verificable} de cada uno ---la migración que creó su tabla o su permiso, la
decisión de arquitectura que lo impuso, el control OWASP, el reglamento de titulación, la
observación docente o el criterio de la asignatura---. Ninguno queda sin origen demostrable.

Escribir además la historia de usuario de esos 33 sigue siendo trabajo deseable, y por eso
permanece en esta sección: una historia añade el «para qué» desde la voz del actor, que una
migración no expresa. Pero es una mejora de la elicitación, no un hueco de trazabilidad: hoy todo
requisito puede demostrar de dónde viene, y la comprobación V6 del validador impide citar una
historia que no exista en disco.
...
\toprule
\textbf{Elaborado por} & \textbf{Revisado y aprobado por (Docente-Director)} \\
\midrule
& \\[1.6cm]
\rule{6cm}{0.4pt} & \rule{6cm}{0.4pt} \\
Álava Alvarado, Jean Pierre & Ing. Guerrero Ulloa Gleiston Cíceron, Mg. \\
\href{https://orcid.org/0009-0001-2878-2919}{ORCID: 0009-0001-2878-2919} & \\
\vspace{0.5cm} & \\
Fecha: \rule{4cm}{0.4pt} & Fecha de aprobación: \rule{4cm}{0.4pt} \\
\bottomrule
\end{tabular}
\end{center}

\vspace{0.4cm}
\noindent\fbox{\parbox{0.98\textwidth}{\textbf{Estado real de la firma (2026-09-11):} la firma
del docente-director \textbf{no se ha obtenido todavía} para esta versión. La v1.0.0 sí fue
revisada por el docente-director, y esta v1.0.1 es la respuesta a esa revisión
(sección~\ref{sec:respuesta}); la aprobación formal queda pendiente de su nueva lectura. No se
fabrica una firma ni se marca esta sección como completa hasta que ocurra realmente, siguiendo la
misma política de integridad aplicada al resto del documento: donde no hay evidencia, se declara
que no la hay.}}
