...
contra el cual construirlo. Alternativa admisible si se decide no implementarlo en esta
...
OBS-11} (\cd{docs/observaciones/OBSERVACIONES.md}), registrada como \emph{parcialmente resuelta}:
la credencial se retiró del \cd{README.md} pero sigue expuesta en otros artefactos del
repositorio.}
...
identificó el problema y la retirada se hizo solo en uno de los cuatro lugares donde la credencial
aparece: convertirlo en requisito con criterio verificable es lo que le pone condición de cierre a
una observación que hoy lleva meses en estado parcial.}
\campo{Criterio de aceptación}{}
\begin{itemize}
...
contraseñas estén en el código.

\textbf{Inventario completo, verificado el 2026-09-11.} La credencial sembrada aparecía en seis
artefactos versionados del repositorio público, no solo en el \cd{README.md} que OBS-11 dio por
corregido: el código que siembra los datos de demostración, la colección de peticiones de
\cd{docs/postman/}, el escenario de ejemplo de \cd{docs/requisitos/historias/HU-01.md}, el
procedimiento de medición de \cd{k6/README.md}, la evidencia reproducible de
\cd{docs/mediciones/sec/owasp/OWASP-AUDIT.md} y el propio registro de OBS-11. \textbf{Los cinco
artefactos de documentación quedan corregidos en esta versión}: toman la credencial de una
variable de entorno o de una variable de colección, y ninguno la reproduce.

\textbf{Lo que sigue abierto, y por qué.} Primero, la siembra en código: retirarla de ahí cambia
el comportamiento del arranque de la aplicación y merece un cambio propio, revisado por separado,
no un retoque de documentación. Segundo, la migración \cd{V11\_\_roles\_y\_privilegios.sql}
crea dos roles de base de datos con contraseña literal; están declaradas como credenciales de
desarrollo, pero una migración ya aplicada no se puede editar sin romper la suma de verificación
de Flyway, así que cerrarlas exige una migración nueva que las rote, no una corrección en su
sitio. Tercero, y esto no lo arregla ningún cambio futuro: \textbf{el historial de Git conserva
todo lo retirado hoy}, de modo que estas credenciales deben considerarse comprometidas y rotarse
en cualquier despliegue real.

Es el hallazgo de seguridad más serio del punto de corte y encabeza el plan de cierre
(\S\ref{sec:pendientes}).}
...
Endpoints reales del backend \dotfill{} 207\\
Clases de prueba en disco \dotfill{} 45\\
Estados: Implementado 16, No cumplido 6, Planificado 5, Verificado 63\\
Must verificados \dotfill{} 39/50 (78\,\%)\\
...
\textbf{Total} \\
\midrule
Requisitos funcionales     & 48 & 14 & 2 & 0 & \textbf{64} \\
Requisitos no funcionales  & 15 &  2 & 3 & 6 & \textbf{26} \\
\midrule
\textbf{Total}             & \textbf{63} & \textbf{16} & \textbf{5} & \textbf{6} &
\textbf{90} \\
\bottomrule
\end{tabular}
...
De los \textbf{50} requisitos de prioridad \emph{Must}, \textbf{39 están verificados
(78,0\,\%)}. Los \textbf{11} restantes se enumeran aquí de forma completa, porque un porcentaje
...
\bottomrule
\caption{Los 11 requisitos \emph{Must} no verificados, con lo que falta en cada uno.}
\end{longtable}

\noindent Cinco de los once se cierran con evidencia o con una prueba; seis exigen cambiar
