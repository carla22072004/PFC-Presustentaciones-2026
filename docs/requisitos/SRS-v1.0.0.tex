...
prioridad \textbf{Must}, \textbf{7 tienen prueba automatizada real (87.5\%, actualizado 2026-08-29)},
no el 100\% que exige el criterio. Esta cifra se calculó ejecutando
\texttt{scripts/validate-traceability.sh} contra el repositorio real, no se estimó. La cifra mejoró
desde el 5/8 (62.5\%) original de la Fase 6 tras añadir \texttt{SolicitudServiceImplTest.java} (RF-02)
y \texttt{CronogramaServiceImplTest.java} (RF-04) en la Fase 10. Solo RF-06 sigue sin verificación
automatizada: está implementado y funciona (verificado manualmente / con Postman), pero carece de un
test automatizado dedicado en el repositorio. Se declara esta brecha explícitamente en vez de forzar
un 100\% falso.
...
RNF-04 & Mantenibilidad & Cobertura JaCoCo $\geq$ 60\%. \textbf{No cumplido}: 24.63\% de líneas real (2026-08-29), en progreso desde 2.83\% (\texttt{docs/mediciones/jacoco/COVERAGE.md}). \\
\bottomrule
\end{tabular}
\end{table}
