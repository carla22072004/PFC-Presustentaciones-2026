...
### 3. `sp_asignar_jurado_masivo` — categoría: actualizaciones masivas
* **Tipo:** `PROCEDURE`
* **Propósito:** Asignación masiva y transaccional de un arreglo de docentes como jurados a un arreglo de solicitudes.
* **Firma SQL real:**
```sql
...
```
* **Parámetros:** `p_solicitud_ids` (IN, BIGINT[]) · `p_docente_ids` (IN, BIGINT[]) · `p_rol` (IN, VARCHAR — código de `roles_jurado`, p. ej. `PRESIDENTE`/`VOCAL`/`SECRETARIO`).
* **Tablas que afecta:** lee `presus.roles_jurado`; escribe (`INSERT ... ON CONFLICT DO UPDATE`) `presus.miembros_tribunal`.
* **Manejo de errores:** `RAISE EXCEPTION` si los arreglos no coinciden en longitud, o si `p_rol` no existe en `roles_jurado`.
* **Invocación real desde Java** (`JuradoRepository.java`):
```java
...
```
* **Flujo real:** `POST /api/v1/jurados/asignar-masivo` en `JuradoController` (rol ADMIN/COORDINADOR).
* **Verificado real:** 3 llamadas reales asignando PRESIDENTE/VOCAL/SECRETARIO a la solicitud 1 → `200`, filas confirmadas en `presus.miembros_tribunal`.
