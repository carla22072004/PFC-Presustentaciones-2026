...
JaCoCo sobre 559 pruebas automatizadas reales (2026-09-06, \texttt{mvn clean verify} sobre Postgres/Redis
reales en Docker, 0 fallos, ver \texttt{docs/mediciones/jacoco/2026-09-06-servicios/}): \textbf{81.03\,\%
de líneas (3,148/3,885) y 64.39\,\% de ramas (1,016/1,578)} a nivel global. La cobertura de líneas supera
con holgura el umbral RNF-04 de 60\,\%; la de ramas global sigue por debajo de 70\,\%, pero las dos capas
de lógica de negocio del backend ya lo superan individualmente (ver más abajo), así que la brecha global
restante se concentra en los componentes transversales de seguridad, declarada explícitamente en la
Sección~\ref{sec:trabajo-futuro} en vez de omitirse.

El avance más relevante está en las dos capas que peor estaban: los controladores pasaron de
8.75\,\%/0\,\% (líneas/ramas) el 30-08 a \textbf{72.00\,\% de líneas (833/1,157) y 77.41\,\% de ramas
(257/332)}, y los servicios pasaron de 59.64\,\%/40.87\,\% el 05-09 a \textbf{87.57\,\% de líneas
(2,176/2,485) y 70.04\,\% de ramas (706/1,008)} el 06-09 --- ambas capas superan ahora el umbral de
70\,\% exigido en líneas \emph{y} en ramas a la vez. Los controladores se cerraron con 158 pruebas nuevas
en 11 clases, priorizando los que exponen procedimientos almacenados (\texttt{Reporte}, \texttt{Solicitud},
\texttt{Jurado}, \texttt{Tutoria}, \texttt{Evaluacion}, \texttt{Cronograma}, \texttt{Tutor}) y los dos
mayores huecos de ramas (\texttt{Catalogo}, con 102 ramas sin ejercitar, y \texttt{Rol}/\texttt{Permiso},
que concentran las salvaguardas de administración). Los servicios se cerraron con 164 pruebas nuevas en
...
\midrule
Desktop & 68.3 & 100 & 100 & 100 \\
Mobile & 61 & 100 & 100 & 100 \\
\bottomrule
\end{tabular}
\caption{Lighthouse, average of 3 runs per profile (2026-09-06, tras optimizar imagen del logo, cabeceras de caché y meta description; Accessibility ya había subido de 89 a 100 el 2026-08-30).}
\end{table}
...
descriptiva. Performance sigue bajo el umbral de 80/100 en ambos perfiles, aunque con una mejora real y
medida en desktop (65$\to$68.3) tras resolver un hallazgo concreto: el logo institucional pesaba 333\,KB
a 2444$\times$2826\,px mostrado en pantalla a solo 68\,px de ancho (redimensionado a 73,7\,KB, $-$78\,\%),
y las cabeceras de caché de nginx forzaban revalidación de red en cada carga incluso para bundles con hash
inmutables. En mobile, pese a que el Largest Contentful Paint mejoró de forma real (7.6--7.8\,s
$\to$ 6.7--6.8\,s), el puntaje agregado no se movió: bajo el \emph{throttling} de CPU 4$\times$ del
perfil mobile, el cuello de botella deja de ser peso de red y pasa a ser tiempo de arranque/ejecución de
JavaScript del framework (Angular + zone.js) --- un problema de arquitectura, no de assets, fuera del
alcance de una corrección puntual. Se declara como brecha real con causa raíz identificada, en vez de
forzar el número --- ver la nota metodológica completa en
...
2026-09-06). Generado por \texttt{scripts/gen-figuras.py} a partir de
