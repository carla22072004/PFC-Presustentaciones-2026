...
\begin{itemize}[nosep]
    ...
    como proxy inverso.
    \item \textbf{Nivel 3 (Componentes, Figura~\ref{fig:c4-componentes}):} 29 controladores REST, organizados por dominio (Auth,
...
\midrule
Adecuación funcional & 8/8 requisitos Must con prueba automatizada dedicada (100\,\%); 12/15 requisitos
totales (80\,\%); matriz de trazabilidad de 15 filas validada contra el código real
...
formato de intercambio estándar en los 29 controladores. \\
Usabilidad & Instrumento SUS \cite{brooke1996} construido y verificado por autotest, pero \textbf{sin
datos reales todavía} ($N=0$) --- declarado explícitamente como brecha en vez de fabricado
(Sección~\ref{sec:evaluacion}). \\
Fiabilidad & 395 pruebas automatizadas reales (0 fallos), \texttt{/actuator/health} como verificación de
...
Mantenibilidad & Arquitectura en capas (controlador/servicio/repositorio) con 29 controladores
...
\texttt{ActaServiceImpl.firmarActa()} que lo invoca vía JPA 2.1 \texttt{@Procedure}
(\texttt{ActaRepository.firmarActaDigital}).
...
// ActaServiceImpl.java
@Override
@Transactional
public Acta firmarActa(Long actaId, String rol, String observacion) {
    // ... validacion de rol y de que el usuario autenticado es
