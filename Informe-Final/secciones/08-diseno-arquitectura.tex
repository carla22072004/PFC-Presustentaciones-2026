...
\begin{itemize}[nosep]
    ...
    como proxy inverso.
    \item \textbf{Nivel 3 (Componentes):} 21 controladores REST, organizados por dominio (Auth,
...
ADR-006 & Despliegue con Docker Compose, imágenes ancladas por digest SHA-256 & Reproducibilidad determinista. Ampliada en la Fase 8 con la decisión de Railway como proveedor de producción (Sección~\ref{sec:implementacion}) \\
ADR-007 & Control de acceso por permisos dinámicos (BD), no roles estáticos en código & Reemplaza el RBAC fijo con \texttt{@PreAuthorize} descrito originalmente en ADR-005 por una verificación contra \texttt{permisos}/\texttt{rol\_permisos}, para que cambiar qué rol puede qué requiera solo datos, no redespliegue \\
\bottomrule
\end{tabular}
\caption{Resumen de los 7 ADR reales del repositorio (\texttt{docs/adr/}).}
\label{tab:adrs}
\end{table}

\textbf{Corrección de integridad relevante:} ADR-006 afirmaba, en su redacción original, que las
